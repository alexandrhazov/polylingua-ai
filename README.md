...
by **Anthropic Claude** (`claude-haiku-4-5`). Designed for zero-cost deployment
...
app/
  ...
  services/ai_tutor.py # Anthropic async client: generate_sentences / evaluate_translation
...
# 2. Configure
cp .env.example .env
#   edit .env: BOT_TOKEN, ANTHROPIC_API_KEY
...
The bot uses `claude-haiku-4-5` — Anthropic's fastest, most cost-effective tier,
...
intended. Responses use **structured JSON outputs** so scores and sentences
parse reliably.
